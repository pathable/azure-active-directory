...
AzureAd.http.call = (method, url, options) => {
  ...
  try {
    ...
    throw new Meteor.Error(
      'azure-active-directory:failed HTTP request',
      err.message,
      details
    );
  }
...
AzureAd.http.getAccessTokensBase = (additionalRequestParams, tenantId, rootUrlFromRequest) => {
  const config = AzureAd.getConfiguration({ tenantId });
